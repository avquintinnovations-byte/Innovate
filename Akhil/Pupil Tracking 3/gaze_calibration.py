            ...
            if seq_state == 'countdown':
                ...
                if done:
                    ...
                    if calibration_round == 0 and CALIBRATION_ROUNDS >= 2:
                        ...
                        # Save calibration points so other programs (e.g. segmentation)
                        # can load them and reconstruct the gaze mapping.
                        try:
                            np.save("gaze_calibration_points.npy",
                                    np.array(points_for_mapping, dtype=np.float64))
                            print("Saved calibration points to gaze_calibration_points.npy")
                        except Exception as e:
                            print(f"Warning: could not save calibration points: {e}")
