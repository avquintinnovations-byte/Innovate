...
// Touch debug overlay (visual confirmation that touch interrupt is working)
void drawTouchOverlay(bool longPressActive, uint32_t heldMs) {
  // Put indicator near the center so it's obvious even when QR text is present.
  const int cx = DISP_W / 2;
  const int cy = DISP_H / 2;

  const uint16_t color = longPressActive ? TFT_GREEN : TFT_RED;
  const int pulse = (millis() / 80) % 6;  // Slow pulse while finger is down
  const int r = 8 + pulse;

  // Crosshair
  const int half = 24;
  tft.drawLine(cx - half, cy, cx + half, cy, color);
  tft.drawLine(cx, cy - half, cx, cy + half, color);
  ...
  if (longPressActive) {
    tft.printf("TOUCH: LONG (%lu ms)", (unsigned long)heldMs);
  } else {
    if (heldMs > 0) {
      tft.printf("TOUCH (%lu ms)", (unsigned long)heldMs);
    } else {
      tft.print("TOUCH");
    }
  }
...
void loop() {
  ...
  // Always show a touch indicator while the finger is down (for debugging).
  // This is drawn after the QR overlay logic so it stays visible during long-press.
  if (touched) {
    const uint32_t heldMs = lastTouchState ? (millis() - touchStartTime) : 0;
    drawTouchOverlay(inLongPress, heldMs);
  }
